<div class="bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-inner dark:bg-gray-700 dark:border-gray-600">
    <div class="flex items-center justify-between mb-4">
        <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">WooCommerce Revenue Account Mapping</h2>
    ...
    @if (!data_get($xeroStatus, 'connected'))
        <div class="p-4 bg-yellow-100 text-yellow-700 rounded-lg text-sm">
            Please connect to Xero first to load the list of available revenue accounts.
        </div>
    @else
        <form wire:submit.prevent="saveRevenueMapping">
            <div class="space-y-4">
                @foreach ($wcRevenueTypes as $gatewayId => $gatewayName)
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-3 bg-gray-50 rounded-lg border border-gray-200">
                        ...
                        <div class="col-span-2">
                            ...
                            >
                                ...
                                @foreach ($xeroAccounts as $account)
            ...
            >
                <span wire:loading.remove wire:target="saveRevenueMapping">Save Revenue Mappings</span>
                <span wire:loading wire:target="saveRevenueMapping">Saving...</span>
            </button>
        </form>
    @endif
</div>